...
void::arxtoolbox_lib::Arxtoolbox::runCipher()
{
	...
	cl_ulong elementsLengthOut =3524577; //(+1 zero rights)
	cl_ulong elementsLengthIn = 3524577;
	cl_uint numberOfThreads;

	cl_ulong dataSizeOutput =sizeof(cl_ulong)*elementsLengthOut;
	cl_ulong dataSizeInput =sizeof(cl_ulong)*elementsLengthIn;

	cl_ulong* inputData = (cl_ulong*)malloc(dataSizeInput);	
	cl_ulong* outputData = (cl_ulong*)malloc(dataSizeOutput);  //Left mask 
	cl_ulong blocksize = 64;
	...
	// 0. CONSTANT MEMORY Blocksize
	cl_mem  bufferBlockSize = clCreateBuffer(context,CL_MEM_READ_ONLY,sizeof(cl_ulong), NULL,&errNum);
	checkErr(errNum, "clCreateBuffer blocksize KernelPairs");
	// 1. CONSTANT MEMORY nrRounds
	cl_mem  bufferRounds = clCreateBuffer(context,CL_MEM_READ_ONLY,sizeof(cl_ulong), NULL,&errNum);
	checkErr(errNum, "clCreateBuffer nrRounds KernelPairs");
	// 2. GLOBAL MEMORY inputData  -right 48 bits
	cl_mem bufferInDataPairs = clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR , dataSizeInput,inputData, &errNum);
	checkErr(errNum, "clCreateBuffer inputData KernelPairs");
	// 3. GLOBAL MEMORY outputdata
	cl_mem bufferOutDataPairs  = clCreateBuffer(context,CL_MEM_READ_WRITE, dataSizeOutput,NULL, &errNum);
	checkErr(errNum, "clCreateBuffer outputData PairsKernel");
	// 4. GLOBAL MEMORY x0  - BITs attached to the right 44 bits
	cl_mem bufferLeftPartWord = clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR , sizeof(cl_ulong),&leftPartWord, &errNum);
	checkErr(errNum, "clCreateBuffer output leftPartWord KernelPairs");

	//Generate KERNEL =================================================

	cl_kernel kernelPairs= clCreateKernel( program,"ARX_TOOLBOX_PAIRS_kernel", &errNum);
	checkErr(errNum, "clCreateKernel(ARX_TOOLBOX_PAIRS_kernel creating)");
	errNum = clSetKernelArg(kernelPairs, 0, sizeof(cl_mem), (void *)&bufferBlockSize);
	checkErr(errNum, "clSetKernelArg(ARX_TOOLBOX_PAIRS_kernel arg 0)");
	errNum |= clSetKernelArg(kernelPairs, 1, sizeof(cl_mem), (void *)&bufferRounds);
	checkErr(errNum, "clSetKernelArg(ARX_TOOLBOX_PAIRS_kernel arg 1)");
	errNum |= clSetKernelArg(kernelPairs, 2, sizeof(cl_mem), (void *)&bufferInDataPairs);
	checkErr(errNum, "clSetKernelArg(ARX_TOOLBOX_PAIRS_kernel arg 2)");
	errNum |= clSetKernelArg(kernelPairs, 3, sizeof(cl_mem), (void *)&bufferOutDataPairs);  // left mask
	checkErr(errNum, "clSetKernelAg(ARX_TOOLBOX_PAIRS_kernel arg 3)");
	errNum |= clSetKernelArg(kernelPairs, 4, sizeof(cl_mem), (void *)&bufferLeftPartWord);
	checkErr(errNum, "clSetKernelArg(ARX_TOOLBOX_PAIRS_kernel arg 5)");

	//====Schedule the upload of data to these buffers in the command queue==============

	//10.0. upload buffer   |  Blocksize - CONSTANT
	errNum = clEnqueueWriteBuffer(queue,bufferBlockSize, CL_TRUE, 0,sizeof(cl_ulong), &nrRounds, 0,NULL,NULL);
	checkErr(errNum, "upload blocksize to buffer)");
	//10.1. upload buffer   |  nrRounds - CONSTANT
	errNum |= clEnqueueWriteBuffer(queue,bufferRounds,CL_TRUE,0,sizeof(cl_ulong),&blocksize,0,NULL,NULL);
	checkErr(errNum, "upload nrRounds to buffer)");
	//10.2. upload buffer      | inputDataFirstRound - GLOBAL
	errNum |= clEnqueueWriteBuffer(queue,bufferInDataPairs,CL_TRUE,0,dataSizeInput,inputData,0,NULL, &k_eventsGenPairsWrite[0]);
	checkErr(errNum, "upload inputData to buffer)");
	//10.4. upload buffer      |  outputDataFirstRound  - GLOBAL
	errNum = clEnqueueWriteBuffer(queue,bufferOutDataPairs ,CL_TRUE,0,dataSizeOutput, outputData,0,NULL, NULL);
	checkErr(errNum, "upload outputData to buffer)");
	//10.5. upload buffer      |  outCounter  - GLOBAL
	errNum = clEnqueueWriteBuffer(queue,bufferLeftPartWord,CL_TRUE,0,sizeof(cl_ulong), &leftPartWord,0,NULL, &k_eventsGenPairsWrite[1]);
	checkErr(errNum, "upload outcounter to buffer)");

	//Wait until writing of data to devices has been completd
	errNum= clWaitForEvents(2, k_eventsGenPairsWrite);
	...
	//Max number of work items that can assignment to Pairs Kernel -> depends on the memory requirements of the kernel.
	//size_t  maxWGPairs;
	//errNum = clGetKernelWorkGroupInfo (	kernelPairs, deviceIDs[0],CL_KERNEL_WORK_GROUP_SIZE,sizeof(maxWGPairs), &maxWGPairs,NULL);
	//cout << "Workgroupsize for Pairs Kernel " <<  maxWGPairs << endl;

	//Check whether CL_KERNEL_LOCAL_MEM_SIZE is less than or equal to CL_DEVICE_LOCAL_MEM_SIZE.
	//TODO

	numberOfThreads = 3524577;
	size_t globalWorkSizePairs[1] = {numberOfThreads};  //work items per workgroup
	size_t localWorkSizePairs[1] = {1}; 		       //workgroups
	errNum = clEnqueueNDRangeKernel(queue,kernelPairs,1, NULL, globalWorkSizePairs,  localWorkSizePairs,0,NULL, &k_eventsGenPairsKernel[0]);
	checkErr(errNum, "Queue kernelFirstRound");

	//==================================================================================
	// 12. Wait for the command queue to run empty, indicating execution has finished
	// Technically don't need this as we are doing a blocking read with in-order queue.
	//clWaitForEvents(events.size(), &events[0]);

	//==================================================================================
	// 13.  Read back the results
	// 1. retreive outputDataFirstRound
	clEnqueueReadBuffer(queue,bufferOutDataPairs,CL_TRUE,0,dataSizeOutput,outputData,0,NULL,NULL); // wait until computation is finished
   
	/*
	///PRINT RESULT
        for (int i = 31900;  i < 32000; i++)
        { 
        //cout << dec << i << "\t";
	cout << hex  << outputData[i] << "\n";
	...
	*/
	...
	cl_mem bufferInputCipher = clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR , dataSizeOutput,outputData, &errNum);
	...
	errNum |= clEnqueueWriteBuffer(queue,bufferInputCipher,CL_TRUE,0,dataSizeInput, outputData,0,NULL, NULL); // Wait unit previous kernel has been finished
	...
	size_t localWorkSizeCal[2] = {32,32};
	...
	saveRoundResultsComboToFile( outputBestOverallForward, "~/words64bitBestOverallResultForward.txt", nrRounds, 1);
	//std::cout << "- done" << endl;
	//std::cout <<  "writing Output for Best overall Result Backward ";
	saveRoundResultsComboToFile(outputBestOverallBackward,"~/words64bitBestOverallBackward.txt", nrRounds, 1);
	//std::cout << "- done" << endl;
	//std::cout <<  "writing Output for Best Round Result Forward ";
	saveRoundResultsComboToFile( outputBestRoundsForward,"~/words64bitBestRoundResultForward.txt", nrRounds, 2);
	// std::cout << "- done" << endl;
	//std::cout <<  "writing Output for Best Round Result Backward ";
	saveRoundResultsComboToFile( outputBestRoundsBackward,"~/words64bitBestRoundResultBackward.txt", nrRounds, 2);
	//std::cout << "- done" << endl;
	//std::cout <<  "writing Output for Best Round Result Combo ";
	saveRoundResultsComboToFile(outputBestRoundCombo,"~/words64bitBestRoundResultCombo.txt", nrRounds, 3);
	//std::cout << "- done" << endl;
