...
{
    return ((((value & 0xffffffff) << (number)) | ((value & 0xffffffff) >> ((wordlength) - number))) & 0xffffffff);;
};
//===============================================================
                                        ...
                                        )
{
    ...
    else if( (((inWords[gidDim1])&0xff0000000)!= 0x180000000 )  & ((inWords[gidDim1] &  0xffffffff ) != 0x0 )  )  // word not zero and pairs are consequtive
    {
 	__private uint blocksize = 64;
	__private uint rounds = 22;
        
        //check if incoming  masks are consequtive
	if ( checkTwoConsecutiveBits( (inWords[(gidDim1)] &  0xffffffff ) ,blocksize)  ) 
         {
