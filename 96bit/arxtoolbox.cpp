...
void::arxtoolbox_lib::Arxtoolbox::runCipher()
{
	...
	cl_ulong elementsLengthOut =75024; //(+1 zero rights)
	cl_ulong elementsLengthIn = 75024;
	...
	cl_ulong word_size = 0xffffff; 	        //Inner loop - pre-generated for the 28 bits
	...
	*/
	...
	//cl_ulong fourthDim =0;
    	for(fourthDim = 0;  fourthDim  <   75024; fourthDim++)
   	{
		cout << "Iterations to go: " <<  (75024-fourthDim) << endl;  
		cout << "Current fourthDim value: " << outputData[fourthDim] << endl; 
		...
		checkErr(errNum, "upload outputBestOverallForward KernelCipher)");
		// 5. GLOBAL MEMORY inputDataFirstCalculation
		errNum |= clEnqueueWriteBuffer(queue,bufferInputCipher,CL_TRUE,0,dataSizeInput, outputData,0,NULL, NULL); 
		checkErr(errNum, "upload inputDataFirstCalculation KernelCipher)");
		// 6. GLOBAL MEMORY inputDataFirstCalculation
		errNum |= clEnqueueWriteBuffer(queue,bufferfourthDim,CL_TRUE,0,sizeof(cl_ulong),&fourthDim,0,NULL, NULL); 
		checkErr(errNum, "upload inputDataFirstCalculation KernelCipher)");
	...
	clReleaseMemObject(bufferLeftPartWord );
   	clReleaseMemObject(bufferfourthDim );
