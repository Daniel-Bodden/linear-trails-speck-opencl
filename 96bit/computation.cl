                                        ...
                                        )
{
    ...
    else if( checkTwoConsecutiveBits( (  ((inWords[(gidDim2)] << 24 ) | inWords[(gidDim1)] ) &  0xffffff ) ,blocksize)  )  // word not zero and pairs are consequtive
    {
	__private uint blocksize = 96;
     
       /************************************************************************************
        **************************CHECK 0x6... and 0x7... ***********************************/
       if (  (((inWords[gidDim1]>>20)&0xf) == 0x6) | (((inWords[gidDim1]>>20)&0xf) == 0x7) ) 
 	 {
	   __private uint initialblock = 23;


	   cipherRoundFunction(
                                
				( (inWords[(gidDim2)] << 23 ) | inWords[(gidDim1)] ),
				0,
                                blocksize,
                                rounds,
  	   ...
  	   cipherRoundFunction(
                               ( (inWords[(gidDim2)] << 23 ) | inWords[(gidDim1)] ),
				( ( inWords[(fourthDim[0])]  << 23 ) | inWords[(gidDim3)] ),
                                blocksize,
                                rounds,
                               outputBestRoundCombo,               //OUTPUT - write the best round combo (forward, backward)
                               outMasksBestRoundBackward,          //OUTPUT - write the best round backward
       ...
       {
	   __private uint initialblock = 24;
	
	   cipherRoundFunction(
                                
				( (inWords[(gidDim2)] << 24 ) | inWords[(gidDim1)] ),
				0,
                                blocksize,
                                rounds,
                               outputBestRoundCombo,               //OUTPUT - write the best round combo (forward, backward)
            ...
            cipherRoundFunction(
                               ( (inWords[(gidDim2)] << 24 ) | inWords[(gidDim1)] ),
				( ( inWords[(fourthDim[0])]   << 24 ) | inWords[(gidDim3)] ),
                                blocksize,
                                rounds,
                               outputBestRoundCombo,               //OUTPUT - write the best round combo (forward, backward)
                               outMasksBestRoundBackward,          //OUTPUT - write the best round backward
